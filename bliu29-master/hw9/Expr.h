...
// Homework 6
// 10/12/2024
// Header for mathematical expressions
...
class Expr {
 public:
  ...
  // clones the shared pointer to the contents
  virtual shared_ptr<Expr> clone() const = 0;
  // replaces any Var (variables by name) with a Num (numerical value)
  // values is a map from var names to num values
  virtual shared_ptr<Expr> setVariables(
      const map<string, double>& values) const = 0;
  virtual string toString() const = 0;
  // Calculates final value for an Expr
